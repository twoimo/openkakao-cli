...
The supervisor also takes an exclusive owner lock so launchd/manual starts cannot run two reply workers. Its atomic status contains the owner identity, database-authoritative mode, source epoch, and readiness; a collision fails closed. These markers are required by the final unattended sender in addition to both opt-ins and the privacy attestation.
