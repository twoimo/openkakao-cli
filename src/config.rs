    ...
    match config.model.privacy_mode.as_deref() {
        ...
        Some("remote_explicit")
            if config.model.allow_egress
                && config.model.provider.as_deref().is_some_and(|v| !v.trim().is_empty())
                && config.model.retention.as_deref().is_some_and(|v| !v.trim().is_empty()) =>
        {
            Ok(())
        }
        Some(mode) => anyhow::bail!("model privacy mode '{mode}' is not eligible for unattended use"),
