...
from auto_reply_ax_ui import CHAT, exact_window_available, snapshot

ROOT = Path(__file__).resolve().parents[1]
def reply_authors() -> set[str]:
    configured = os.environ.get("OPENKAKAO_REPLY_AUTHORS", "").strip()
    return {name.strip() for name in configured.split(",") if name.strip()}
HOOK = ROOT / "scripts" / "auto-reply-worker.py"
...
STATE = Path(
    os.environ.get(
        ...
        str(Path.home() / "Library/Application Support/openkakao/auto-reply/apple-watch-state.json"),
    )
)
STATUS = Path(
    os.environ.get(
        "OPENKAKAO_APPLE_WATCH_STATUS",
        str(Path.home() / "Library/Application Support/openkakao/auto-reply/apple-watch-status.json"),
    )
)
