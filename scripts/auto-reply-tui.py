#!/usr/bin/env python3
"""Read-only real-time terminal dashboard for AutoReply auto replies."""
...
from auto_reply_transition_journal import (
...
def _room_has_monitor_state(entry: Path) -> bool:
    return any(
        (entry / name).is_file()
        for name in (
            "supervisor-status.json",
            "reply-worker-status.json",
            "reply-queue.sqlite3",
            "db-watch-state.json",
        )
    )
...
def _discover_rooms(state_root: Path, requested: Iterable[int]) -> list[Path]:
    ...
    for entry in rooms_root.iterdir():
        ...
        if requested_set and room_id not in requested_set:
            continue
        # Instant GeekNews/auto-reply-now may mkdir a catalog target and leave
        # only operator-request.json. That sidecar is not a monitored worker.
        if not requested_set and not _room_has_monitor_state(entry):
            continue
        try:
...
def _plain(snapshot: dict[str, Any]) -> str:
    ...
    lines = [
        "AutoReply Auto Reply Dashboard (read-only)",
...
def _draw(
    ...
    _add(screen, 0, 0, " AutoReply · Auto Reply Control Center ", header_style)
...
def main() -> int:
    ...
    parser.add_argument(
        "--state-root",
        type=Path,
        default=Path.home() / "Library/Application Support/openkakao/auto-reply",
    )
    ...
    try:
        ...
        print(f"auto-reply-tui: {exc}", file=sys.stderr)
        return 2
