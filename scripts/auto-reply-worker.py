...
def leftover_pre_send_unknown_skip_fields(
    ...
    if _row_attempt_no(row) >= BLANK_UNKNOWN_MAX_ATTEMPTS:
        # Blank delivery_unknown reopen is for a fresh crash, not leftover
        # inbound that has already been claimed hundreds of times. In-slot
        # GeekNews still reopens pending so the window is not burned.
        if _proactive_unix_leftover_job(row) or not _row_event_is_proactive(row):
            return {
                "status": "skipped",
                "decision": "skip",
            ...
            AND (
              (
                ...
                AND (
                  reason IS NULL
                  OR reason = ''
                  OR reason IN ('model_usage_limited', 'model_rate_limited', 'model_authentication_unavailable', 'model_temporarily_unavailable', 'stale_backlog', 'conversation_advanced', 'burst_superseded', 'author_not_allowlisted', 'self_author', 'author_identity_drift', 'retrieval_command_failed')
                )
              )
              OR (
                reply IS NOT NULL
...
def _policy_valid_draft(
    ...
    *,
    ...
    if not text or len(text) > 220:
        return False
    if not (
        _outbound_reaction_allows(text, inbound)
        and _outbound_question_allows(text, inbound)
        and _outbound_register_allows(text, recipient=recipient, register=register)
    ):
        return False
    ...
    if _outbound_echoes_inbound(text, inbound):
        return False
    if _outbound_echoes_share_preview(text, inbound, link_previews or []):
        return False
...
def _rerank_query_text(
    ...
    for row in reversed(list(recent_conversation or [])):
        if not isinstance(row, dict) or row.get("is_self") is True:
            continue
        message = str(row.get("message") or "").strip()
        if not message or message in {"이겅", "[사진]", "[파일]"}:
            continue
        author = str(row.get("author_nickname") or "").strip()
        recent_bits.append(f"{author} {message}" if author else message)
...
def select_ranked_reply(
    ...
    *,
    ...
    for item in [preferred, *drafts]:
        ...
        if text and text not in candidates and _policy_valid_draft(
            text,
            inbound,
            recent_conversation,
            link_previews,
    ...
    *,
    ...
    try:
        ...
        payload = {
            ...
            "recent_conversation": list(analysis.get("recent_conversation") or [])[:13],
...
def _try_youtube_oembed(watch_url: str, deadline: float) -> tuple[dict, int] | None:
    ...
    if not title:
        return None
    text = " ".join(part for part in (title, author) if part)[:MAX_LINK_TEXT_CHARS]
    return (
        {
            "url": watch_url,
            "title": title,
    ...
    try:
        ...
        if marker is not None:
            ...
            if (
                ...
                return False
        elif not path.name.startswith("auto-reply-ax-image-"):
            return False
...
def _reply_laughter_policy_allows(reply: str, inbound: str | None = None) -> bool:
    ...
    if "ㅎ" in reply:
        return False
    return all(len(match.group(0)) >= 3 for match in re.finditer(r"ㅋ+", reply))
...
def _partner_streak_already_answered(
    ...
    humans = [
        ...
        if row.get("is_self") is not True
    ]
    if not humans:
        return False
    ...
    for index, row in reversed(humans):
        ...
        if sent is not None:
            last_sent = sent
    return any(
        row.get("is_self") is True and index > start_index
        for index, row in enumerate(rows)
    )
...
def _merge_recent_sent_self(
    ...
    merged = list(rows)
    for item in extra:
        if not isinstance(item, dict) or item.get("is_self") is not True:
            continue
...
def _inbound_asks_question(text: str) -> bool:
    ...
    return bool(
        ...
        or re.search(r"(알려줘|설명해|답변해|말해줘|대답해)\s*$", body)
    )
...
def _identity_policy_decision(message: str) -> dict | None:
    if _inbound_accuses_bot(message):
        return {
            "should_reply": True,
            "reply": AI_TELL_PROBE,
            "reason": "identity_tell_probe",
            "category": "social",
            "evidence_ids": [reaction_register()["evidence_id"]],
            "drafts": [AI_TELL_PROBE],
        }
...
def _outbound_question_allows(reply: str, inbound: str = "") -> bool:
    ...
    return compact == AI_TELL_PROBE and _inbound_accuses_bot(inbound)
...
DEFAULT_REPLY_INSTRUCTIONS = tuple([
            ...
            "Reply at most once to the other person's current burst. If recent_conversation already has a 최연우 message after they started this burst, set should_reply false with reason already_commented unless they asked a question or directed the line at 최연우.",
            "Sound like 최연우 talking to this recipient. With 현준, that is casual 존댓말, still short and specific, never 반말. With 문승현 or other informal recipients, stay casual and a bit blunt. Prefer a short specific take over padded agreement. Do not start a reply with 응. Do not open with ㅇㅇ unless the rest of the bubble adds a new fact or action.",
            ...
            "If inbound accuses this account of being AI/봇/자동응답/기계, reply with exactly 어디가 봇 같았음? and nothing else. Do not deny or confirm being human.",
            ...
            "Use prior_reply_decisions to avoid repeating the same take. Silence is correct when this account already commented on this burst.",
            ...
            "Keep the reply under 220 characters. Prefer one short KakaoTalk line (about 8–20 characters) like 최연우 in this room. Two lines only when splitting one thought.",
            ...
            "Inspect every supplied image and every fetched link preview. Never pretend to have seen pixels or a page that was not retrieved.",
...
def _operator_state_root() -> Path:
    raw = os.environ.get("OPENKAKAO_AUTO_REPLY_STATE_ROOT", os.environ.get("OPENKAKAO_BUJAMENTOR_STATE_ROOT", "")).strip()
    if raw:
        return Path(raw)
...
def generate_reply(
    ...
    *,
    ...
    if REPLY_RUNNER_KIND == "codex":
        ...
        gjc_agent_dir = _operator_state_root() / "gjc-agent"
        if gjc_agent_dir.is_dir():
            env["GJC_CODING_AGENT_DIR"] = str(gjc_agent_dir)
            env["PI_CODING_AGENT_DIR"] = str(gjc_agent_dir)
    codex_stdin_prefix = (
        system_prompt
    ...
    for attempt in range(PRE_SEND_PREFLIGHT_ATTEMPTS):
        ...
        if (
            ...
            leftover_retry = (
                ...
                and bool(event_id)
                and leftover_unknown_has_ax_mutation(connection, event_id)
...
def analyze_event(event: dict) -> dict:
    ...
    try:
        ...
        if urls and event.get("proactive") is not True and not links_fully_retrieved(
        ...
        if ranked.get("fallback") == "all_policy_rejected" or not result["reply"]:
            result["decision"] = "skip"
            result["reason"] = (
                "all_policy_rejected"
...
def defer_scheduled_pre_send_unavailable(
    ...
    *,
    ...
    is stale. Empty replies may still expire after the grace window; a formed
    reply keeps retrying until it sends, matches later self, or is skipped for
    ...
    if connection is not None:
        ...
        if row is not None:
            try:
                formed_reply = str(row["reply"] or "").strip()
            except (KeyError, IndexError, TypeError):
                formed_reply = str(row[0] or "").strip()
    ...
    if deadline <= current:
        ...
        if current >= grace_until and not formed_reply:
