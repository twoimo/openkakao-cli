...
# See docs/auto-reply-launchd-supervision.md.
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --chat)
      [ "$CHAT_SET" -eq 0 ] || die "managed service accepts at most one --chat; use [auto_reply].chats for multiple rooms"
      CHAT="${2-}"
      CHAT_SET=1
      shift 2
      ;;
...
def identity_path(raw: str, *, python: bool = False) -> str:
    if python and raw in HOMEBREW_OPT_PYTHON:
        return raw
    return os.path.realpath(raw)

expected = {
    "python": identity_path(sys.argv[2], python=True),
    "entry": identity_path(sys.argv[3]),
    "binary": identity_path(sys.argv[4]),
    "config": identity_path(sys.argv[5]),
    "chat_selectors": normalize_selector(sys.argv[6]) if sys.argv[7] == "1" else [],
    "state_root": identity_path(sys.argv[8]),
}
...
def owned_file(raw: str, executable: bool) -> pathlib.Path:
    path = pathlib.Path(raw)
    keg = str(path) in HOMEBREW_OPT_PYTHON_PATHS
    if not path.is_absolute() or (path.is_symlink() and not keg):
        raise SystemExit(f"unsafe file: {path}")
    resolved = path.resolve(strict=True)
    metadata = resolved.stat()
    if (
        not stat.S_ISREG(metadata.st_mode)
        or (not keg and metadata.st_uid != os.geteuid())
        or stat.S_IMODE(metadata.st_mode) & 0o022
    ):
    ...
    return path if keg else resolved
