...
PLUTIL="${OPENKAKAO_AUTO_REPLY_PLUTIL:-plutil}"
MODE=""
BIN=""
HEALTH_BIN=""
HOOK_PATH=""
...
usage() {
  cat <<'EOF'
usage:
  install-auto-reply-launchd.sh --mode preflight --bin ABS --health-bin ABS [--state-root ABS]
  install-auto-reply-launchd.sh --mode production --bin ABS --health-bin ABS --hook-path ABS [--state-root ABS]
EOF
}
