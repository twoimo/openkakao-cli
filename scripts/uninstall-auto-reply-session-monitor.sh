...
LEGACY_LABEL="com.openkakao.bujamentor.session-monitor"
STATE_ROOT="${HOME}/Library/Application Support/openkakao/auto-reply"
if [ ! -f "${STATE_ROOT}/enrollment.json" ] \
   && [ -f "${HOME}/Library/Application Support/openkakao/bujamentor/enrollment.json" ]; then
...
if "$LAUNCHCTL" print "$LEGACY_SERVICE" >"$OUTPUT" 2>&1; then
  if ! "$LAUNCHCTL" bootout "$LEGACY_SERVICE" >"$OUTPUT" 2>&1; then
    /bin/cat "$OUTPUT" >&2 || true
    die "failed to boot out $LEGACY_SERVICE; disable sentinel remains installed"
  fi
fi
