...
HEALTH_BIN_PATH="${TMP}/openkakao-auto-reply-health"
...
reset_case() {
  ...
  STATE_ROOT="${HOME_DIR}/Library/Application Support/openkakao/auto-reply"
  LAUNCH_AGENTS_DIR="${HOME_DIR}/Library/LaunchAgents"
  rm -rf "$HOME_DIR" "$FAKE_STATE_DIR"
  mkdir -p "$HOME_DIR" "$FAKE_STATE_DIR"
  : > "$LAUNCHCTL_LOG"
  : > "$PLUTIL_LOG"
  OPENKAKAO_AUTO_REPLY_FAIL_BOOTSTRAP=
  OPENKAKAO_AUTO_REPLY_FAIL_KICKSTART=
  OPENKAKAO_AUTO_REPLY_FAIL_PRINT=
  OPENKAKAO_AUTO_REPLY_FAIL_BOOTOUT=
}
...
run_install() {
  ...
  OPENKAKAO_AUTO_REPLY_LAUNCHCTL="${FAKE_BIN_DIR}/launchctl" \
  OPENKAKAO_AUTO_REPLY_PLUTIL="${FAKE_BIN_DIR}/plutil" \
  OPENKAKAO_AUTO_REPLY_FAIL_BOOTSTRAP="${OPENKAKAO_AUTO_REPLY_FAIL_BOOTSTRAP-}" \
  OPENKAKAO_AUTO_REPLY_FAIL_KICKSTART="${OPENKAKAO_AUTO_REPLY_FAIL_KICKSTART-}" \
  OPENKAKAO_AUTO_REPLY_FAIL_PRINT="${OPENKAKAO_AUTO_REPLY_FAIL_PRINT-}" \
  OPENKAKAO_AUTO_REPLY_FAIL_BOOTOUT="${OPENKAKAO_AUTO_REPLY_FAIL_BOOTOUT-}" \
  sh "${ROOT}/scripts/install-auto-reply-launchd.sh" \
    --mode "$mode" \
    --bin "$BIN_PATH" \
...
run_uninstall() {
  ...
  FAKE_LAUNCHCTL_STATE_DIR="$FAKE_STATE_DIR" \
  OPENKAKAO_AUTO_REPLY_LAUNCHCTL="${FAKE_BIN_DIR}/launchctl" \
  sh "${ROOT}/scripts/uninstall-auto-reply-launchd.sh" --state-root "$STATE_ROOT" "$@"
}
...
if action == "bootstrap":
    ...
    if service in fail_set("OPENKAKAO_AUTO_REPLY_FAIL_BOOTSTRAP"):
    ...
    if service in fail_set("OPENKAKAO_AUTO_REPLY_FAIL_KICKSTART"):
    ...
    service = args[1]
    if service in fail_set("OPENKAKAO_AUTO_REPLY_FAIL_PRINT"):
    ...
    service = args[1]
    if service in fail_set("OPENKAKAO_AUTO_REPLY_FAIL_BOOTOUT"):
...
for script in \
  scripts/install-auto-reply-launchd.sh \
  scripts/uninstall-auto-reply-launchd.sh \
  scripts/status-auto-reply-launchd.sh \
  scripts/doctor-auto-reply-launchd.sh \
  scripts/test-auto-reply-launchd-artifacts.sh
 do
  sh -n "${ROOT}/$script"
done
...
if OPENKAKAO_AUTO_REPLY_FAIL_PRINT="$HEALTH_SERVICE" run_install_preflight; then
...
if OPENKAKAO_AUTO_REPLY_FAIL_PRINT="$WATCH_SERVICE" run_install_production; then
