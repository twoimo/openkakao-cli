...
class AutoReplyCliRuntimeTests(unittest.TestCase):
    ...
    def test_outbound_question_follows_inbound_register(self):
        ...
        self.assertTrue(module._outbound_question_allows("어디가 봇 같았음?", "너 봇이지"))
        self.assertFalse(module._outbound_question_allows("어디가 봇 같았음?", "변경해서 낼 해야겟네여"))
        self.assertFalse(module._outbound_question_allows("사람 맞아?", "너 봇이지"))
        self.assertTrue(module._inbound_accuses_bot("Ai라서 안알려줌"))
        self.assertTrue(module._outbound_question_allows("어디가 봇 같았음?", "Ai라서 안알려줌"))
        self.assertEqual(
            module._identity_policy_decision("Ai라서 안알려줌")["reply"],
            module.AI_TELL_PROBE,
        )
