...
TUI = ROOT / "scripts" / "auto-reply-tui.py"
...
class AutoReplyTuiTests(unittest.TestCase):
    maxDiff = None
    ...
    def fixture(self, root: Path):
        module = load(f"auto_reply_tui_{id(root)}")
        state = root / "state"
        rooms = state / "rooms"
        room = rooms / "42"
    ...
    def test_missing_or_empty_room_state_never_reports_zero_of_zero_ready(self):
        with tempfile.TemporaryDirectory() as temporary:
            module = load(f"auto_reply_tui_missing_rooms_{id(temporary)}")
    ...
    def test_operator_request_only_room_is_not_monitored(self):
        with tempfile.TemporaryDirectory() as temporary:
            module = load(f"auto_reply_tui_opreq_{id(temporary)}")
            _tui, state, room, _queue, now = self.fixture(Path(temporary))
            ...
            snapshot = module.collect_snapshot(state.resolve(), now=now)
            chat_ids = [item.get("chat_id") for item in snapshot.get("rooms") or []]
            self.assertNotIn(260330955968694, chat_ids)
            self.assertIn(int(room.name), chat_ids)

    def test_operator_request_only_directory_is_ignored_like_empty_rooms(self):
        with tempfile.TemporaryDirectory() as temporary:
            module = load(f"auto_reply_tui_opreq_empty_{id(temporary)}")
            state = Path(temporary).resolve() / "state"
            rooms = state / "rooms"
            leftover = rooms / "260330955968694"
            state.mkdir(mode=0o700)
            rooms.mkdir(mode=0o700)
            leftover.mkdir(mode=0o700)
            self._private_json(
    ...
    def test_durable_timeline_survives_restart_is_newest_first_and_scrolls(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            restarted_module = load(f"auto_reply_tui_restart_{id(temporary)}")
